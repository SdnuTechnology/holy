...
- 🎯 Entry points to execute your program using `python -m <holy>` or `$ holy` with basic CLI argument parsing.
...
---
# holy Flask Application

Awesome holy created by SdnuTechnology
...
```bash
git clone https://github.com/SdnuTechnology/holy holy
cd holy
make install
```
...
```bash
holy create-db   # run once
holy populate-db  # run once (optional)
holy add-user -u admin -p 1234  # ads a user
holy run
```
